 ...
 */
function camer_get_svg( $icon = null ) {
	...
	if ( 'primary' === $args->theme_location || 'secondary' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ( 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title = $title . '<span class="sub-menu-icon">' . camer_get_svg( 'expand' ) . '</span>';
			}
		}
	}
...
function camer_social_icons_menu( $item_output, $item, $depth, $args ) {
	...
	if ( 'top-social' === $args->theme_location ) {
		...
		$item_output = str_replace( $args->link_after, '</span>' . camer_get_social_svg( $icon ), $item_output );
	}
